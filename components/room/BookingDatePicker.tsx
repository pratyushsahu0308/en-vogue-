  ...
  return (
    <div className="booking-card shadow p-4">
      <p className="price-per-night">
        <b>${room?.pricePerNight}</b> / day
      </p>

      <hr />

      <p className="mt5 mb-3">Select dates</p>
      ...
      {isAvailable === true && (
        <div className="alert alert-success my-3">
          Ticket is available. Book now.
        </div>
      )}
      {isAvailable === false && (
        <div className="alert alert-danger my-3">
          Ticket not available. Try different dates.
        </div>
      )}

      {isAvailable && !isAuthenticated && (
        <div className="alert alert-danger my-3">Login to book ticket.</div>
      )}
